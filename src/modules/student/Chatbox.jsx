...
const Chatbox = () => {
  ...
  const getEnhancedBotResponse = async (userInput) => {
    ...
    for (const [key, data] of Object.entries(knowledgeBase)) {
      if (key !== 'default' && data.patterns.some(pattern => inputLower.includes(pattern))) {
        const responses = data.responses;
        return responses[Math.floor(Math.random() * responses.length)];
      }
    }
  ...
  return (
    <div className="chatbox-container">
      {isOpen && (
        <div className={`chatbox-popup ${isMobile ? 'mobile-view' : ''}`}>
          <div className="chatbox-header">
            <div className="header-content">
              <h3>Course Assistant</h3>
              <div className="status">
          ...
          <div className="chatbox-messages">
            {messages.map((message, index) => (
              <div key={index} className={`message ${message.sender} ${isMobile ? 'mobile-message' : ''}`}>
                <div className="message-content">
                  {message.text.split('\n').map((paragraph, i) => (
                    <p key={i}>{paragraph}</p>
                  ))}
          ...
          <div className={`quick-questions ${isMobile ? 'mobile-quick-questions' : ''}`}>
            <p>Quick questions:</p>
            <div className={`quick-questions-grid ${isMobile ? 'mobile-grid' : ''}`}>
              {commonQuestions.map((question, index) => (
                <button 
                  key={index} 
                  onClick={() => handleQuickQuestion(question)}
                  className={`quick-question-btn ${isMobile ? 'mobile-quick-btn' : ''}`}
                >
                  {question}
                </button>
              ))}
          ...
          <form onSubmit={handleSubmit} className={`chatbox-input ${isMobile ? 'mobile-input' : ''}`}>
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              ...
              required
            />
            <button type="submit" className={isMobile ? 'mobile-send-btn' : ''}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="20px" height="20px">
                <path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/>
              </svg>
            </button>
      ...
      <button onClick={toggleChatbox} className={`chatbox-icon ${isMobile ? 'mobile-icon' : ''}`}>
        {isOpen ? (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="24px" height="24px">
            <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
          </svg>
