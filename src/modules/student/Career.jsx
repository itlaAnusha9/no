...
//       metrics: [
//         { name: t('metrics.totalQuizzes'), value: quizResults.totalQuizzes, max: 50, trend: 'up' },
//         { name: t('metrics.averageScore'), value: parseFloat(quizAverage), unit: '%', max: 100, trend: 'up' },
//         { name: t('metrics.totalQuestions'), value: quizResults.totalQuestions, max: 500, trend: 'steady' }
//       ],
//       details: {
...
//         ],
//         history: getQuizHistory(), // Dynamic history from context
//         chartData: {
...
//       icon: <Clock size={24} />,
//       metrics: [
//         { name: t('metrics.totalTests'), value: mockTestResults.totalTests, max: 10, trend: 'up' },
//         { name: t('metrics.averageScore'), value: parseFloat(mockAverage), unit: '%', max: 100, trend: 'up' },
//         { name: t('metrics.totalQuestions'), value: mockTestResults.totalQuestions, max: 1000, trend: 'steady' }
//       ],
//       details: {
...
//         ],
//         history: getMockHistory(), // Dynamic history from context
//         chartData: {
...
//     if (showDetails.id === 'quiz') return getQuizHistory();
//     if (showDetails.id === 'mock') return getMockHistory();
//     return [];
//   };
...
//                         {getHistory().slice(-5).reverse().map((item) => ( // Show last 5, newest first
...
//   const { t } = useTranslation();
//   const navigate = useNavigate();
//   const { quizResults, mockTestResults, getQuizHistory, getMockHistory, getQuizResults } = useQuiz();
...
//       const currentQuizResults = getQuizResults ? getQuizResults() : null;
     
//       if (currentQuizResults && typeof currentQuizResults === 'object') {
//         setSafeQuizResults({
//           totalScore: currentQuizResults.score || 0,
//           totalQuestions: currentQuizResults.totalQuestions || 0,
//           totalQuizzes: currentQuizResults.totalQuizzes || (currentQuizResults.totalQuestions > 0 ? 1 : 0),
//           byLevel: currentQuizResults.byLevel || {}
//         });
//       }
...
//     }
//   }, [getQuizResults, mockTestResults]);
...
const Career = () => {
  ...
  useEffect(() => {
    try {
      // Set current quiz results
      if (quizResults && typeof quizResults === 'object') {
        setSafeQuizResults(quizResults);
      }
 
      // Set mock results safely
      if (mockTestResults && typeof mockTestResults === 'object') {
        setSafeMockResults(mockTestResults);
      }
    } catch (error) {
      console.error('Error initializing quiz results:', error);
      // Keep default values if there's an error
