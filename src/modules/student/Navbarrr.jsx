...
//   // MARK: UPDATED - Load study plans and notifications from localStorage with duplicate prevention
//   useEffect(() => {
//     loadStudyPlans();
//     loadNotifications();
...
//       if (event.detail && event.detail.studyPlan) {
//         // Check for duplicates before adding
//         const isDuplicate = studyPlans.some(plan => plan.id === event.detail.studyPlan.id);
//         if (!isDuplicate) {
//           addStudyPlan(event.detail.studyPlan);
//         }
//       }
//     };
...
//       if (event.detail && event.detail.notification) {
//         // Check for duplicates before adding
//         const isDuplicate = notifications.some(notif => notif.id === event.detail.notification.id);
//         if (!isDuplicate) {
...
//       window.removeEventListener('notificationAdded', handleNotificationAdded);
//     };
//   }, [studyPlans, notifications]);

//   // MARK: UPDATED - Study plan functions with improved duplicate prevention
//   const loadStudyPlans = () => {
//     try {
//       const savedPlans = localStorage.getItem('studyPlans');
//       if (savedPlans) {
...
//       id: notificationId,
//       type: 'study_plan_created',
//       title: t('notifications.new_study_plan_created'),
//       message: t('notifications.study_plan_added_to_calendar', { title: studyPlan.title }),
//       date: new Date().toISOString(),
//       read: false,
//       planId: studyPlan.id
//     };
...
//   return (
//     <motion.nav
//       className={`navbar ${scrolled ? 'navbar-scrolled' : ''} ${isMobile ? 'navbar-mobile' : ''}`}
//       initial={{ y: -100 }}
//       animate={{ y: 0 }}
//       transition={{ duration: 0.6 }}
//     >
...
//           >
//             <img src={novyaLogo} alt="NOVYA Logo" style={{ height: isMobile ? '30px' : '35px' }} />
//             <motion.span
//               style={{
//                 background: 'linear-gradient(90deg, #2D5D7B 0%, #4a8db7 25%, #FF6B6B 50%, #FFD166 75%, #2D5D7B 100%)',
//                 WebkitBackgroundClip: 'text',
...
//                       <div style={{ marginTop: '16px', borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
//                         <h4 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '8px', color: '#1f2937' }}>
//                           {t('calendar.study_plans_for')} {selectedDate.toLocaleDateString(i18n.language)}
//                         </h4>
//                         {getStudyPlansForDate(selectedDate).length === 0 ? (
//                           <p style={{ fontSize: '12px', color: '#6b7280', textAlign: 'center', margin: '12px 0' }}>
//                             {t('calendar.no_study_plans_scheduled')}
//                           </p>
//                         ) : (
//                           <div style={{ maxHeight: '120px', overflow: 'auto' }}>
//                             {getStudyPlansForDate(selectedDate).map((plan, index) => (
...
//                           <div style={{ padding: '32px 16px', textAlign: 'center', color: '#6b7280' }}>
//                             <FaBell size={24} style={{ marginBottom: '8px', opacity: 0.5 }} />
//                             <p style={{ margin: 0, fontSize: '14px' }}>{t('notifications.no_notifications_yet')}</p>
//                             <p style={{ margin: '4px 0 0 0', fontSize: '12px' }}>{t('notifications.study_plans_will_appear_here')}</p>
//                           </div>
//                         ) : (
//                           notifications.map((notification) => (
//                             <div
...
//                       }}>
//                         <FaCoins size={14} color="#FFA500" />
//                         <span>{t('profile.reward_points')}: {currentRewardPoints.toLocaleString()}</span>
//                       </div>
//                       <div style={{
//                         fontSize: '12px',
//                         color: '#8B5A2B',
...
//         ) : (
//           /* Mobile Menu Toggle */
//           <div className="mobile-menu-toggle" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
//             {/* Mobile Reward Points (Simplified) */}
//             <div style={{
//               display: 'flex',
//               alignItems: 'center',
//               gap: '6px',
...
//             {/* User Info */}
//             <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '15px', background: '#f8fafc', borderRadius: '12px', marginBottom: '20px' }}>
//               <div
//                 style={{
//                   width: '40px',
//                   height: '40px',
...
//               </div>
//               <div>
//                 <div style={{ fontWeight: '600', color: '#1f2937' }}>{name || 'User'}</div>
//                 <div style={{ fontSize: '12px', color: '#6b7280' }}>{t('profile.reward_points')}: {currentRewardPoints}</div>
//               </div>
//             </div>
...
//                       fontWeight: '500',
//                       background: activeLink === link.path ? '#f0f7ff' : 'transparent'
//                     }}
//                   >
//                     {link.icon}
//                     <span>{link.name}</span>
//                     {link.hasDropdown && <FaChevronDown size={12} style={{ marginLeft: 'auto' }} />}
//                   </Link>
...
//                       borderRadius: '6px',
//                       cursor: 'pointer',
//                       color: i18n.language === lang.code ? '#2D5D7B' : '#374151',
...
//             >
//               {/* Calendar content same as desktop but adapted for mobile */}
//               <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
...
//               onClick={(e) => e.stopPropagation()}
//             >
//               {/* Notifications content same as desktop but adapted for mobile */}
//               <div style={{
//                 padding: '16px',
//                 borderBottom: '1px solid #e5e7eb',
//                 display: 'flex',
